...
void set_tissue_settings_idealised(Cell_parameters p, Tissue_parameters *t)
{
	...
	else if (strcmp(t->Tissue_model, "basic") == 0)
	{
		// Settings
	...
	// Conduction velocity ==================================================\\|
	else if (strcmp(t->Tissue_model, "conduction_velocity") == 0)
	{
		// Global params
	...
	// Idealised re-entry ===================================================\\|
	else if (strcmp(t->Tissue_model, "re-entry") == 0 || strcmp(t->Tissue_model, "re-entry_large") == 0)
	{
		// Global params
		t->dx = t->dy = t->dz = 0.25;    // mm
		t->D1   = 0.2;                  // mm^2/ms
    ...
    else if (strcmp(t->Tissue_model, "Vent_transmural") == 0) // Transmural strand/slice/slab through ventricular wall
    {
        // Global params
    ...
    else if (strcmp(t->Tissue_model, "Heterogeneous_human_atria") == 0) // Strand/tissue with all human atrial celltypes
    {
		// Global params
		...
		{
			t->het_junction_X_location[1] = i*t->NX/t->Ncelltypes; // evenly distribute celltypes within geometry
		}
...
void overwrite_tissue_properties_from_args(Cell_parameters p, Tissue_parameters *t, Argument_parameters A)
{
    ...
    if (strcmp(t->Orientation_type, "isotropic") != 0 && strcmp(t->Orientation_type, "anisotropic") != 0) 
    {
        printf("ERROR! Orientation_type %s is invalid; must be \"isotropic\" or \"anisotropic\"\n", t->Orientation_type);
    }
...
{
    if (strcmp(t.Tissue_order, "geo") == 0) sc->N = read_geo_file(sc, sc->geo, t.geo_file, "Tissue_geometries", PATH, Output_dir, "anatomy");  // read geo || lib/Spatial_coupling.cc
