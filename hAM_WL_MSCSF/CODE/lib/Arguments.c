...
void set_arguments(int Narg, char *argin[], Argument_parameters *A, char const *Version, FILE *out)
{
	...
	while (counter < Narg) // loop until Number of arguments has been reached
	{
        ...
        if (strcmp(argin[counter], "Spatial_output_range_start") == 0)
        {
            A->SORs            = atoi(argin[counter+1]);
            A->SORs_arg        = true;
            fprintf(out, "Spatial_output_range_start   %s ", argin[counter+1]);
            counter++; isFound = true;
        }    
        if (strcmp(argin[counter], "Spatial_output_range_end") == 0)
        {
            A->SORe            = atoi(argin[counter+1]);
            A->SORe_arg        = true;
            fprintf(out, "Spatial_output_range_end   %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        ...
        {
            printf("ERROR: \"%s\" functionality is not included in this version. Please check https://github.com/michaelcolman/ for releases which include this functionality (which may only be released after publication of related works)\n\n", argin[counter]);
            exit(1);
        ...
        if (strcmp(argin[counter], "LTCC_redist") == 0)
        {
            printf("ERROR: \"%s\" functionality is not included in this version. Please check https://github.com/michaelcolman/ for releases which include this functionality (which may only be released after publication of related works)\n\n", argin[counter]);
            exit(1);
            A->LTCC_redist        = argin[counter+1];
            A->LTCC_redist_arg    = true;
            fprintf(out, "LTCC_redist %s ", argin[counter+1]);
            if (strcmp(A->LTCC_redist, "On") != 0 && strcmp(A->LTCC_redist, "Off") != 0)
            {
                printf("ERROR: \"%s\" is not a valid LTCC_redist argument. Please pass only \"Off\" or \"On\"\n\n", A->LTCC_redist);
                exit(1);
            }
            counter++; isFound = true;
        }
		if (strcmp(argin[counter], "TT_map_file") == 0)
        {
            ...
            fprintf(out, "TT_map_file %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        ...
        if (strcmp(argin[counter], "RyR_het") == 0)
        {
            ...
            {
                printf("ERROR: \"%s\" is not a valid RyR_het argument. Please pass only \"Off\" or \"random\"\n\n", A->RyR_het);
                exit(1);
            }
            counter++; isFound = true;
        }
        ...
        if (strcmp(argin[counter], "LTCC_het") == 0)
        {
            ...
            {
                printf("ERROR: \"%s\" is not a valid LTCC_het argument. Please pass only \"Off\" or \"random\"\n\n", A->LTCC_het);
                exit(1);
            }
            counter++; isFound = true;
        }
        ...
        if (strcmp(argin[counter], "volds_het") == 0)
        {
            ...
            if (strcmp(A->volds_het, "Off") != 0 && strcmp(A->volds_het, "random") != 0)
            {
                printf("ERROR: \"%s\" is not a valid volds_het argument. Please pass only \"Off\" or \"random\"\n\n", A->volds_het);
                exit(1);
            }
            counter++; isFound = true;
        }
        ...
        if (!isFound)
		{
			...
			if (strcmp(Version, "Tissue_native") == 0 || strcmp(Version, "Tissue_integrated") == 0)
			{
				...
				printf("\tSpatial_output_interval_{vtk/data} [int ms]\t Spatial_output_range_{start/end} [int ms]\n");
			...
			if (strcmp(Version, "Single_cell_3D") == 0)
            {
				...
				//printf("\tDetub [On/Off]\tTT_map_file [string]\tLTCC_redist [On/Off]\n");
                ...
                printf("\tRyR_het [Off/random]\tLTCC_het [Off/random]\tvolds_het [Off/random]\n");
